...
def test_get_url(url_mock_response):
    ...
    assert "https://www.googleapis.com/calendar/v3/calendars/u8n1onpbv9pb5du7gssv2md58s@group.calendar.google.com" \
...
def test_ceil_dt():
    ...
    assert dt + (datetime.datetime(datetime.MINYEAR, 1, 1, tzinfo=datetime.timezone.utc) - dt) % delta \
           == gcal.ceil_dt(dt, delta)
