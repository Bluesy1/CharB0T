...
def test_wrong_channel_error():
    ...
    assert str(error).encode("ascii", "ignore").decode() == "This command can only be run in the channel <#123456789> ."
...
def test_no_pool_found_default_message():
    ...
    assert (
        error.message.encode("ascii", "ignore").decode()
        == "test pool not found. Please choose one from the autocomplete."
    )
    assert isinstance(error, AppCommandError)
...
def test_missing_single_program_role():
    ...
    assert (
        str(error).encode("ascii", "ignore").decode() == "You are missing at least one of the required roles: 'role1'"
...
def test_missing_multiple_program_roles():
    ...
    assert (
        str(error).encode("ascii", "ignore").decode()
        == "You are missing at least one of the required roles: 'role1' or 'role2'"
        " - you must be at least level 1 to use this command/button."
    )
    assert isinstance(error, AppCommandError)
