...
class TestDice:
    ...
    @pytest.mark.asyncio
    async def test_valid_roll_async(self):
    ...
    @pytest.mark.asyncio
    async def test_invalid_roll_async(self):
        mock_ctx = Mock(spec=commands.Context)
        mock_bot = Mock(spec=commands.Bot)
        mock_ctx.author.mention = "mock"
        cog = dice.Roll(mock_bot)
