...
async def friday_5(bot: commands.Bot) -> None:
    ...
    if (
        ...
        and datetime.datetime.now(tz=datetime.timezone.utc).hour == 1
        and datetime.datetime.now(tz=datetime.timezone.utc).minute == 0
    ):
        await (await bot.fetch_channel(878434694713188362)).send(
            random.choice(
                [
                    "Bork!",
                    "Bork!",
