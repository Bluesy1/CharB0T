...
def main():  # pylint: disable=too-many-statements
    ...
    @lightbulb.add_checks(
        has_roles(832521484378308660, 832521484378308659, 832521484378308658, mode=any)
    )
    ...
    async def on_guild_message(event: GuildMessageCreateEvent):
        ...
        if used_slurs != set() and not any(
            rid in event.member.role_ids
            for rid in [832521484378308660, 832521484378308659, 832521484378308658]
        ):
            ...
            await bot.rest.create_message(
                832521484828147741,
                embed=Embed(
                    title=f"[SLUR] {event.member.username}#{event.member.discriminator}",
                    color="0xff0000",
                    timestamp=datetime.now(tz=timezone.utc),
                )
                .add_field("User", event.member.mention, inline=True)
                .add_field(
                    "Slurs Used", f"||{joinstring.join(used_slurs)}||", inline=True
                )
                .add_field("Channel", f"<#{event.channel_id}>", inline=True)
                .add_field("Message", event.content, inline=True),
            )
