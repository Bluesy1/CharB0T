    ...
    if event.content is not None and event.is_human:
        ...
        if any([has_whatsapp,has_dollarsign,has_nums]):
            if all([has_whatsapp,has_dollarsign,has_nums]):
                ...
                embed = Embed(title="Log Level Event - only Phone Numbertrigger hit:",description=event.content,color="0x0000ff").add_field("Phone Number Check:",f"Triggered: Found Regex Match(s):{nums}" if has_nums else "No Regex Matches Present",inline=True).add_field("Member",f"Username: {event.member.username}, Discriminator: {event.member.discriminator}",inline=True).add_field("Member ID",event.member.id,inline=True).add_field("Channel:",event.get_channel().mention,inline=True)
                await EventsPlugin.app.rest.create_message(926532222398369812,embed=embed)
