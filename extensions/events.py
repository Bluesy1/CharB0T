...
async def on_guild_message(event: hikari.GuildMessageCreateEvent) -> None:
    ...
    if event.content is not None and event.is_human:
        if not any(role in event.member.role_ids for role in [338173415527677954, 253752685357039617, 225413350874546176, 387037912782471179, 406690402956083210, 729368484211064944]):#pylint: disable=using-constant-test, line-too-long
