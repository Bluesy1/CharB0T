...
async def sensitive_scan(event: GuildMessageCreateEvent) -> None:
    ...
    for word in fulldict["words"]:
        if word in event.content.lower():
            count_found += 1
            used_words.add(word)
    EVENTS.d.last_sensitive_logged.setdefault(
        event.author_id, datetime.now() - timedelta(days=1)
    )
    if datetime.now() > (
        EVENTS.d.last_sensitive_logged[event.author_id] + timedelta(minutes=5)
    ) and (
        (
            count_found >= 2
            and 25 <= (len(event.content) - len("".join(used_words))) < 50
        )
        or (count_found > 2 and (len(event.content) - len("".join(used_words))) >= 50)
        or (count_found >= 1 and (len(event.content) - len("".join(used_words))) < 25)
    ):
        ...
        embed.add_field(
            "Author:",
            f"{event.member.display_name}: {event.author.username}#{event.author.discriminator}",
            inline=True,
        )
        embed.add_field(
            "Message Link:",
            f"[Link]({event.message.make_link(event.guild_id)})",
            inline=True,
        )
        if event.message.channel_id == 926532222398369812:
            await event.message.respond(embed=embed)
        if (await event.message.fetch_channel()).parent_id in (
            360818916861280256,
            942578610336837632,
        ):
            return
        ...
        await webhook.execute(
            username=bot_user.username, avatar_url=bot_user.avatar_url, embed=embed
        )
...
async def on_guild_message(event: hikari.GuildMessageCreateEvent) -> None:
    ...
    if event.content is not None and event.is_human:
        ...
        if not any(
            ...
            if (
                f"<@&{event.guild_id}>" in event.content
                or "@everyone" in event.content
                or "@here" in event.content
            ):
                ...
                embed = Embed(
                    ...
                    text=f"Sent by {event.message.member.display_name}-{event.message.member.id}",
        ...
        if False and re.search(
            r"bruh", event.content, re.IGNORECASE
        ):  # pylint: disable=condition-evals-to-constant
            await event.message.delete()
        elif re.search(
            r"~~:.|:;~~", event.content, re.MULTILINE | re.IGNORECASE
        ) or re.search(
...
async def on_error(event: lightbulb.CommandErrorEvent) -> None:
    ...
    if isinstance(event.exception, lightbulb.CommandInvocationError):
        await event.context.respond(
            ...
            f"Bluesy#8150 has been notified.",
            flags=64,
        )
    ...
    if isinstance(exception, lightbulb.NotOwner):
        ...
        if any(
            substring in str(exception)
            for substring in [
                "check_author_is_me",
                "punished",
                "check_author_work_allowed",
                "check_author_is_admin",
                "check_author_is_mod",
            ]
        ):
            await event.context.respond(
                "You are not authorized to use this command", flags=64
            )
