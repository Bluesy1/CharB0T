...
async def on_message(event) -> None:
    if event.is_human and not user.roleCheck(event.message.member, [338173415527677954,253752685357039617,225413350874546176,387037912782471179,406690402956083210,729368484211064944]):
...
async def on_error(event: lightbulb.CommandErrorEvent) -> None:
    ...
    if isinstance(exception, lightbulb.NotOwner):
        ...
        if "check_author_is_me" in str(exception) or "Punished" in str(exception):
