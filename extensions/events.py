    ...
    async def sensitive_scan(self, message: discord.Message) -> None:
        ...
        if datetime.now() > (self.last_sensitive_logged[message.author_id] + timedelta(minutes=5)) \
            ...
            embed = Embed(title="Probable Sensitive Topic Detected", description=f"Content:\n {message.content}",
                          color=Color.red(), timestamp=datetime.now(tz=timezone.utc))
            embed.add_field(name="Words Found:", value=", ".join(used_words)[0:1024], inline=True)
            embed.add_field(name="Author:", value=f"{message.author.display_name}: "
                                                  f"{message.author.name}#{message.author.discriminator}",
                            inline=True)
            embed.add_field(name="Message Link:", value=f"[Link]({message.message.make_link(message.guild_id)})",
                            inline=True)
            if message.message.channel_id == 926532222398369812:
                await message.channel.send(embed=embed)
            if message.channel.category.category_id in (360818916861280256, 942578610336837632):
                return
            if message.channel.id == 837816311722803260:
                return
            await webhook.send(username=bot_user.name, avatar_url=bot_user.avatar.url, embed=embed)
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if message.content is not None and not message.author.bot:
            ...
            if not any(role.id in [338173415527677954, 253752685357039617, 225413350874546176, 387037912782471179,
                ...
                if f"<@&{message.guild.id}>" in message.content or "@everyone" in message.content or "@here" in \
                        message.content:
                    await message.author.add_roles(discord.Object(id=676250179929636886),
                                                   discord.Object(id=684936661745795088))
                    await message.delete()
                    channel = await self.bot.fetch_channel(426016300439961601)
                    embed = (Embed(description=message.content, title="Mute: Everyone/Here Ping sent by non mod",
                                   color=Color.red())
                             .set_footer(text=f"Sent by {message.author.display_name}-{message.author.id}",
