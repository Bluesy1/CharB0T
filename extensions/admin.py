...
AdminPlugin = lightbulb.Plugin(
    "AdminPlugin", default_enabled_guilds=[225345178955808768]
)
AdminPlugin.add_checks(
    has_roles(338173415527677954, 253752685357039617, 225413350874546176, mode=any)
)
...
@lightbulb.command(
    "maketime",
    "makes a unix time constructor for use with discord's unix time feature",
...
async def command(ctx):  # pylint: disable=too-many-statements
    ...
    (
        ...
        .add_option("Done", "Done")
        .set_emoji("✅")
        .set_description("Use this to get the final options")
        .add_to_menu()
        .add_option("+30 minutes", "+m")
        .set_emoji("➡️")
        .set_description("Use this to increase the time by 30 minutes")
        .add_to_menu()
        .add_option("-30 minutes", "-m")
        .set_emoji("⬅️")
        .set_description("Use this to decrease the time by 30 minutes")
        .add_to_menu()
        .add_option("+1 hour", "+h")
        .set_emoji("➡️")
        .set_description("Use this to increase the time by 1 hour")
        .add_to_menu()
        .add_option("-1 hour", "-h")
        .set_emoji("◀️")
        .set_description("Use this to decrease the time by 1 hour")
        .add_to_menu()
        .add_option("-1 day", "-d")
        .set_emoji("⬅️")
        .set_description("Use this to decrease the time by 1 day")
        .add_to_menu()
        .add_option("+1 day", "+d")
        .set_emoji("▶️")
        .set_description("Use this to increase the time by 1 day")
        .add_to_menu()
        .add_option("+1 week", "+w")
        .set_emoji("➡️")
        .set_description("Use this to increase the time by 1 week")
        .add_to_menu()
        .add_option("-1 week", "-w")
        .set_emoji("◀️")
        .set_description("Use this to decrease the time by 1 week")
        .add_to_menu()
        .add_to_container()
    )
    add_buttons = AdminPlugin.bot.rest.build_action_row()
    add_buttons.add_button(ButtonStyle.SUCCESS, "+w").set_label("+1 week").set_emoji(
        "▶️"
    ).add_to_container()
    add_buttons.add_button(ButtonStyle.SUCCESS, "+d").set_label("+1 day").set_emoji(
        "▶️"
    ).add_to_container()
    add_buttons.add_button(ButtonStyle.SUCCESS, "+h").set_label("+1 hour").set_emoji(
        "▶️"
    ).add_to_container()
    add_buttons.add_button(ButtonStyle.SUCCESS, "+m").set_label(
        "+30 minutes"
    ).set_emoji("▶️").add_to_container()
    add_buttons.add_button(ButtonStyle.PRIMARY, "Done").set_label("Done").set_emoji(
        "✅"
    ).add_to_container()
    sub_buttons = AdminPlugin.bot.rest.build_action_row()
    sub_buttons.add_button(ButtonStyle.DANGER, "-w").set_label("-1 week").set_emoji(
        "◀️"
    ).add_to_container()
    sub_buttons.add_button(ButtonStyle.DANGER, "-d").set_label("-1 day").set_emoji(
        "◀️"
    ).add_to_container()
    sub_buttons.add_button(ButtonStyle.DANGER, "-h").set_label("-1 hour").set_emoji(
        "◀️"
    ).add_to_container()
    sub_buttons.add_button(ButtonStyle.DANGER, "-m").set_label("-30 minutes").set_emoji(
        "◀️"
    ).add_to_container()
    ...
    embed = Embed(
        title="Time builder",
        description=f"timestamp: {time_seconds}: <t:{time_seconds}:F>",
    )
    await ctx.respond(
        "Use the select menu to adjust the time",
        embed=embed,
    ...
    try:
            ...
            async for event in stream:
                ...
                if key == "Done":
                    embed = (
                        ...
                        .add_field(
                            f"Default: `<t:{time_seconds}>`", f"<t:{time_seconds}>"
                        )
                        .add_field(
                            f"Short Time: `<t:{time_seconds}:t>`",
                            f"<t:{time_seconds}:t>",
                        )
                        .add_field(
                            f"Long Time: `<t:{time_seconds}:T>`",
                            f"<t:{time_seconds}:t>",
                        )
                        .add_field(
                            f"Short Date: `<t:{time_seconds}:d>`",
                            f"<t:{time_seconds}:d>",
                        )
                        .add_field(
                            f"Long Date: `<t:{time_seconds}:D>`",
                            f"<t:{time_seconds}:D>",
                        )
                        .add_field(
                            f"Short Date/Time: `<t:{time_seconds}:f>`",
                            f"<t:{time_seconds}:f>",
                        )
                        .add_field(
                            f"Long Date/Time: `<t:{time_seconds}:F>`",
                            f"<t:{time_seconds}:F>",
                        )
                        .add_field(
                            f"Relative Time: `<t:{time_seconds}:R>`",
                            f"<t:{time_seconds}:R>",
                        )
                    )
                    await ctx.edit_last_response(
                        f"<t:{time_seconds}", embed=embed, components=[]
                    )
                    return
                if key == "+w":
                    time_seconds += 604800
        ...
        await ctx.edit_last_response(
            "Waited for 15 seconds... Timeout.", embed=None, components=[]
        )
