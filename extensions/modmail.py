...
from helper import (
    ...
    EVERYONE_MODMAIL,
    user_perms,
    make_modmail_buttons,
    blacklist_user,
    un_blacklist_user,
    get_blacklist,
    check_not_modmail_blacklisted,
)

MODMAIL_PLUGIN = lightbulb.Plugin(
    "Modmail_Plugin",
            ...
            if interaction.custom_id in [
                ...
                channel = await event.app.rest.create_guild_text_channel(
                    ...
                    permission_overwrites=[
                        MOD_GENERAL,
                        EVERYONE_MODMAIL,
                        user_perms(interaction.user.id),
                    ],
                )
                ...
                if interaction.guild_id is None:
                    try:
                        ...
                        await channel.send(
                            MODMAIL_PLUGIN.d.message_dict[interaction.message.id]
                        )
                        del MODMAIL_PLUGIN.d.message_dict[interaction.message.id]
                    finally:
                        await interaction.edit_message(
                            interaction.message.id, "Understood.", components=[]
                        )
                else:
                    await channel.send(
                        f"<@{interaction.user.id}> please send your query/message here.",
                        user_mentions=[interaction.user.id],
                    )
                ...
                if interaction.guild_id is None:
                    ...
                    await channel.send(
                        f"This is a **PRIVATE** modmail channel for <@{interaction.user.id}>"
...
@lightbulb.command(
    "modmail", "modmail command group", auto_defer=True, ephemeral=True, hidden=True
)
...
@lightbulb.option(
    "add", "True to add to blacklist, False to remove", type=bool, required=True
)
@lightbulb.command(
    "edit_blacklist",
    "modmail command group",
...
async def modmail_edit_blacklist(ctx: SlashContext) -> None:
    ...
    if ctx.options.add and successful:
        await ctx.respond(
            f"<@{ctx.options.user.id}> successfully added to the blacklist"
        )
        ...
        await ctx.respond(
            f"<@{ctx.options.user.id}> successfully removed from the blacklist"
        )
    elif not ctx.options.add and not successful:
        await ctx.respond(
            f"<@{ctx.options.user.id}> was not on the blacklist or was not able to be removed from it."
        )
    else:
        await ctx.respond(
