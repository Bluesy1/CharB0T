...
MODVADMIN: Final[str] = (
    "A  role marked as mod is ignored by autopunishments. A role marked as admin can edit the "
    "bot's settings along with the benefits of being a mod. "
)
DISABLER: Final[str] = (
    "A role marked as disabling will mean that a user with this role cannot use this portion of "
...
ECONOMYSETTINGS: Final[str] = (
    "Minimum Gain, Maximum Gain, and Step dictate how random numbers are generated for the "
...
MODERATIONSETTINGS: Final[str] = (
    "Nitro Scan and Crypto Scan respectively refer to whether the Nitro and Crypto Scam "
...
async def check_author_work_allowed(context: lightbulb.Context) -> bool:
    ...
    mydb = await get_db()
    result = await mydb.fetch(
        "SELECT * FROM guild_feature_work_roles WHERE guild_id = $1", context.guild_id
    )
...
async def check_author_is_admin(context: lightbulb.Context) -> bool:
    ...
    mydb = await get_db()
    result = await mydb.fetch(
        "SELECT * FROM guild_mod_roles WHERE guild_id = $1", context.guild_id
    )
    role_ids = context.member.role_ids
    for sub_result in result:
...
@lightbulb.option(
    "group",
    "settings group to query",
    type=int,
    choices=[
        CommandChoice(name="Work Roles", value=1),
        CommandChoice(name="Work settings", value=2),
    ],
)
@lightbulb.command(
    "query", "querys guild settings for the bot", inherit_checks=True, auto_defer=True
)
...
async def config_mods_query(ctx: lightbulb.Context):
    ...
    if queryier == 1:
        result = await mydb.fetch(
            "SELECT * FROM guild_feature_work_roles WHERE guild_id = $1", ctx.guild_id
        )
        embed = Embed(
        ...
        for sub_result in result:
            embed.add_field(
                "Disabling Role" if sub_result["disabling"] else "Enabling Role",
                f"<@&{sub_result['role_id']}>",
                inline=True,
            )
    elif queryier == 2:
        result = await mydb.fetchrow(
            "SELECT * FROM guild_feature_work WHERE guild_id = $1", ctx.guild_id
        )
        embed = Embed(
            title=f"Economy settings in {ctx.get_guild().name}",
            description=ECONOMYSETTINGS,
            timestamp=datetime.datetime.now(tz=datetime.timezone.utc),
            color="0x0000ff",
        )
        embed.add_field(
            "Minimum Gain", f"{result['min_gain']} {result['coin_symbol']}", inline=True
        )
        embed.add_field(
            "Maximum Gain", f"{result['max_gain']} {result['coin_symbol']}", inline=True
        )
        embed.add_field(
            "Step", f"{result['gain_step']} {result['coin_symbol']}", inline=True
        )
        embed.add_field(
            "Gain Cooldown",
            f"{result['gain_cooldown']} Seconds ({result['gain_cooldown'] / 3600} Hours)",
            inline=True,
        )
        ...
        embed.add_field(
            "Starting Balance",
            f"{int(result['starting_bal'])} {result['coin_symbol']}",
            inline=True,
        )
    else:
        return
...
@lightbulb.option(
    "add", "True to add or edit, False to remove", type=bool, required=True
)
@lightbulb.option(
    "role", "role to add/edit/remove from work list", type=hikari.Role, required=True
)
@lightbulb.command(
    "work",
    "adds, edits, or removes a role from consideration as a work role",
    inherit_checks=True,
    auto_defer=True,
)
@lightbulb.implements(commands.SlashSubCommand)
async def config_roles_work(ctx: lightbulb.Context):
    ...
    if add and not new:
        await mydb.execute(
            "UPDATE guild_feature_work_roles set disabling = $1 WHERE guild_id = $2 and role_id = $3",
            disabling,
            ctx.guild_id,
            role.id,
        )
        ...
        await mydb.execute(
            "INSERT INTO guild_feature_work_roles (guild_id, role_id, disabling) VALUES ($1,$2, $3)",
            ctx.guild_id,
            role.id,
            disabling,
        )
        ...
        await mydb.execute(
            "DELETE FROM guild_feature_work_roles WHERE role_id = $1", role.id
        )
        result = await mydb.fetch(
            "SELECT * FROM guild_feature_work_roles WHERE guild_id = $1", ctx.guild_id
        )
    embed = Embed(
        title="New List of enabling and disabling roles",
        description=MODVADMIN,
        color="0x00ff00"
        if add and new
        else "0x0000ff"
        if add and not new
    ...
    if add and new:
        embed.add_field(
            "**NEW** Disabling Role" if bool(disabling) else "**NEW** Enabling Role",
            f"<@&{role.id}>",
            inline=True,
        )
    elif add and not new:
        embed.add_field(
            "**CHANGED TO** Disabling Role"
            if bool(disabling)
            else "**CHANGED TO** Enabling Role",
            f"<@&{role.id}>",
            inline=True,
        )
    for sub_result in result:
...
@lightbulb.option(
    "starting_bal",
    "balance to start all new users of the bot on the server with",
    type=int,
    default=1,
    min_value=1,
    required=False,
...
@lightbulb.option(
    "name",
    "custom name for the coin, put 'default' (without the quotes) to reset to the default name",
    type=str,
    default="",
    required=False,
)
...
@lightbulb.option(
    "step",
    "step between numbers possible to randomly generate",
    type=int,
    default=1,
    min_value=1,
    max_value=100,
...
@lightbulb.option(
    "maximum",
    "maximum possible gain for a use of /work",
    type=int,
    default=1,
    min_value=1,
    required=False,
...
@lightbulb.option(
    "minimum",
    "minimum possible gain for a use of /work",
    type=int,
    default=0,
    min_value=0,
    required=False,
...
@lightbulb.command(
    "work",
    "edits guild settings for the work module. leave option default to leave unchanged",
    inherit_checks=True,
    auto_defer=True,
)
@lightbulb.implements(commands.SlashSubCommand)
async def config_work(ctx: lightbulb.Context):
    ...
    if all(
        ...
        for thing in [
            ...
            ctx.options.maximum,
            ctx.options.step,
            ctx.options.cooldown,
            ctx.options.name,
            ctx.options.symbol,
            ctx.options.starting_bal,
        ]
    ):
        await ctx.respond(
            embed=Embed(
    ...
    starting_bal = (
        ctx.options.starting_bal if ctx.options.starting_bal is not None else 1
    )
    mydb = await get_db()
    result = await mydb.fetchrow(
        "SELECT * FROM guild_feature_work WHERE guild_id = $1", ctx.guild_id
    ...
    if curr_min > curr_max:
        ...
        return
    embed = Embed(
        title=f"**New** Economy settings in {ctx.get_guild().name}",
        description=ECONOMYSETTINGS,
        timestamp=datetime.datetime.now(tz=datetime.timezone.utc),
        color="0x00ff00",
    )
    if minimum:
        await mydb.execute(
            "UPDATE guild_feature_work set min_gain = $1 WHERE guild_id = $2",
            minimum,
            ctx.guild_id,
        )
    embed.add_field(
        f"{'**NEW**' if minimum else ''} Minimum Gain",
        f"{minimum if minimum else result['min_gain']} {symbol if symbol else result['coin_symbol']}",
        inline=True,
    )
    if maximum != 1:
        await mydb.execute(
            "UPDATE guild_feature_work set max_gain = $1 WHERE guild_id = $2",
            maximum,
            ctx.guild_id,
        )
    embed.add_field(
        f"{'**NEW**' if maximum != 1 else ''} Maximum Gain",
        f"{maximum if maximum != 1 else result['max_gain']} {symbol if symbol else result['coin_symbol']}",
        inline=True,
    )
    if step != 1:
        await mydb.execute(
            "UPDATE guild_feature_work set gain_step = $1 WHERE guild_id = $2",
            step,
            ctx.guild_id,
        )
    embed.add_field(
        f"{'**NEW**' if step != 1 else ''} Step",
        f"{step if step != 1 else result['gain_step']} {symbol if symbol else result['coin_symbol']}",
        inline=True,
    )
    if cooldown != 3599:
        await mydb.execute(
            "UPDATE guild_feature_work set gain_cooldown = $1 WHERE guild_id = $2",
            cooldown,
            ctx.guild_id,
        )
    try:
    ...
    if name and name != "default":
        await mydb.execute(
            "UPDATE guild_feature_work set coin_name = $1 WHERE guild_id = $2",
            name,
            ctx.guild_id,
        )
    if name and name != "default":
    ...
    embed.add_field(
        f"{'**NEW**' if name else ''} Coin Name",
        name
        if name and name != "default"
        else DEFAULTNAME
        if name == "default"
        else result["coin_name"],
        inline=True,
    )
    if symbol and symbol != "default":
        await mydb.execute(
            "UPDATE guild_feature_work set coin_symbol = $1 WHERE guild_id = $2",
            symbol,
            ctx.guild_id,
        )
    embed.add_field(
        f"{'**NEW**' if symbol else ''} Coin Symbol",
        symbol
        if symbol and symbol != "default"
        else DEFAULTSYMBOL
        if symbol == "default"
        else result["coin_symbol"],
        inline=True,
    )
    if starting_bal != 1:
        await mydb.execute(
            "UPDATE guild_feature_work set starting_bal = $1 WHERE guild_id = $2",
            starting_bal,
            ctx.guild_id,
        )
    embed.add_field(
...
async def work(ctx: lightbulb.Context):  # pylint: disable=too-many-locals
    ...
    if (
        await mydb.fetchrow(
            "SELECT * FROM user_guild_balance WHERE user_id = $1 AND guild_id = $2",
            ctx.member.id,
            ctx.guild_id,
        )
        is None
    ):
        temp = await mydb.fetchrow(
            "SELECT starting_bal FROM guild_feature_work WHERE guild_id = $1",
            ctx.guild_id,
        )
        await mydb.execute(
    ...
    symbol = guild_info[4]
    time_difference = (
        (current_use - last_work.astimezone(timezone("US/Eastern")))
        if last_work is not None
        else datetime.timedelta(seconds=seconds)
    )
    if time_difference < datetime.timedelta(seconds=seconds):
        last_work: datetime.datetime
        time_pre_tuple = last_work.astimezone(
...
async def balance(ctx: lightbulb.Context):
    ...
    symbol = (
        await mydb.fetchrow(
            "SELECT coin_symbol FROM guild_feature_work WHERE guild_id = $1",
            ctx.guild_id,
        )
    )[0]
    await ctx.respond(
        embed=Embed(
            description=f"{ctx.member.display_name}, your current balance is {user_guild_balance} {symbol}",
            color="60D1F6",
        ),
        flags=EPHEMERAL,
    )
