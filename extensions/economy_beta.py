...
async def work(ctx:lightbulb.Context):
    ...
    if timeDifference < datetime.timedelta(seconds=seconds):
        await ctx.respond(f"🚫 Error: **{ctx.author.mention}** come back <t:{round(time.mktime((currentUse + datetime.timedelta(seconds=seconds) - timeDifference).timetuple()))}:R> to use this command.",flags=EPHEMERAL)
