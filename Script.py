...
async def Coins(message, user):
    ...
    for role in [338173415527677954,253752685357039617,225413350874546176]:
        if discord.utils.get(message.guild.roles, id=int(role)) in message.author.roles:
            ...
            isAllowed
    if isAllowed == False:
        await message.author.send("<:KSplodes:896043440872235028> Error: you are not authorized to use this command.")
    else:
        if message.message.mentions != []:
            df = pd.read_csv(UserListURL, index_col=0)
            funds = df.loc[message.message.mentions[0].id, 'Coin Amount']
            embeddict = {'color': 6345206, 'type': 'rich', 'description': '<@!'+ str(message.message.mentions[0].id) + '> has ' + str(funds) + '<:HotTips2:465535606739697664>'}
            await message.author.send(embed=discord.Embed.from_dict(embeddict))
        elif message.message.mentions == []:
            df = pd.read_csv(UserListURL, index_col=0)
            funds = df.loc[int(user), 'Coin Amount']
            embeddict = {'color': 6345206, 'type': 'rich', 'description': '<@!'+ str(user) + '> has ' + str(funds) + '<:HotTips2:465535606739697664>'}
            await message.author.send(embed=discord.Embed.from_dict(embeddict))
